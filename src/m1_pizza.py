...
def draw_points_on_circle(window, circle, number_of_points, color):
    ...
    for k in range(len(points)):
        ...
        point_circle = rg.Circle(points[k], 10)
...
def run_test_polygon():
    ...
    polygon(window, circle, 4, 'green', 10)
...
def run_test_fancy_polygon():
    ...
    circle = rg.Circle(rg.Point(240, 165), 150)
    circle.fill_color = 'yellow'
    fancy_polygon(window, circle, 51, 25, 'black', 1)
    window.close_on_mouse_click()
...
def fancy_polygon(window, circle, number_of_lines, hops_to_next_point, color, thickness):
    ...
    for k in range(len(points)):
        point1 = points[k]
        point2 = points[hops_to_next_point]
        ...
        hops_to_next_point = hops_to_next_point + 1

        if hops_to_next_point == len(points):
            hops_to_next_point = 0
